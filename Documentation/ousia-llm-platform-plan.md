...
**Physical remount to vertical (VG4v4-style bracket) 2026-08-13**: card moved to vertical mount to let the case close. PCIe link initially showed `Speed 2.5GT/s (downgraded), Width x8 (downgraded)` — reseating the riser fixed the speed (now clean `16GT/s`, the 3090's own native max, since it's a Gen4 card). Width staying at `x8` is **not a fault** — the X870E Taichi Lite's two CPU-attached slots run x8/x8 by board design whenever both are populated (same as validated during the July dual-5070Ti build). Thermal soak with the case now closed: 180s `gpu-burn` at stock 350W peaked at **87°C** (vs. 78°C in the original open-case 60s test) with a mild ~4% throughput dip near the end (early sign of thermal-driven clock backoff, well short of the 95°C slowdown/98°C shutdown thresholds — not dangerous, but the top-fan order is the right call).

**Power-capped to 275W 2026-08-13** (owner decision — PSU headroom + thermal margin ahead of the fan upgrade landing): `nvidia-smi -pl 275`, made persistent via a `nvidia-power-limit.service` systemd unit (`After=nvidia-persistenced.service`, `ExecStart=/usr/bin/nvidia-smi -pl 275`, enabled at boot) — verified to survive a real reboot, not just a live-session change. Re-ran the same 180s `gpu-burn` soak at the new cap: **peaked at 83°C and plateaued there** (vs. 87°C and still climbing at 350W), draw pinned right at the 275W limit throughout, 0 errors. Meaningful thermal margin gained for the interim before the top fans arrive.

**VM RAM rebalanced 2026-08-13** — `pneuma` 40G→22G, `ousia` 12G→30G (owner decision, to fix the "RAM: still not fixed" gap flagged in Phase 1). Snapshotted both VMs first (`pre-ram-rebalance-20260813`). **Incident during the change**: applied both `qm set --memory` config changes, then rebooted `ousia` first while `pneuma` was still running its *old* 40G allocation — briefly requested 40G (old pneuma) + 30G (new ousia) = 70G against furina's ~60G physical, which appears to have OOM-thrashed the **host itself** (kernel/ping still responded, but SSH hung mid-handshake — classic memory-thrashing signature, not a crash; dmesg ring buffer didn't survive the eventual hard reboot so this is inferred, not directly confirmed). Owner did a physical power cycle to recover. **Lesson: when rebalancing memory between VMs on a host with tight headroom, shrink the VM giving up memory *before* growing the one receiving it** — don't let both changes' worst case overlap even briefly.

Post-recovery: both VMs came back at their correct new allocations automatically (`pneuma` `onboot:1` auto-started; `ousia` needed a manual `qm start`), power cap on `ousia` survived intact (275W, confirms the persistence unit is robust across a hard host reboot too, not just a graceful VM reboot). One side effect: **`ousia`'s SSH host key regenerated** — Proxmox regenerates the cloud-init instance-id on every `qm set`, which makes cloud-init treat the next boot as a "new instance" and rerun its init modules (including SSH host key generation). Benign here, just needed `ssh-keygen -R <ip>` to clear the stale `known_hosts` entry. Worth pinning the cloud-init instance-id (`cicustom`/vendor-data) at some point if this gets annoying — not urgent.

Final state verified healthy: `pneuma` 22G (GPU 37°C, fine), `ousia` 30G (GPU 39°C, 275W cap intact, 27GB free RAM — the headroom this whole exercise was for), k0s node `Ready`.
